...
본 문서(v2)는 v1 "범위 외(Section 6)"로 분류된 **중장기 과제 5건**을 심층 연구하여 구체적 해결 방안을 설계한다.
...
```
주차 1-2: Tier 1-2 (10 hooks, ~26 케이스)
주차 3:   Tier 3 (3 hooks, ~15 케이스)
주차 4:   Tier 4 (3 hooks, ~9 케이스)
주차 5-6: Tier 5 (6 hooks, ~30 케이스)
주차 7+:  컴포넌트 테스트 (선택)
```

**최종 목표**: ~163 케이스 (현재 83 + 추가 80) → 커버리지 약 50%
...
## 4. Phase C: Next.js nonce CSP (F3)
...
```
Phase A  Expo 55 업그레이드              ← ✅ 승인, 즉시 실행 (30분)
    ↓
Phase B  Hook/컴포넌트 테스트            ← ✅ 승인, Tier 1-2부터 점진적 (7주)
    ↓
Phase C  Next.js nonce CSP              ← 보류 (Hobby 플랜, SRI 안정화 대기)
    ↓
...
**독립 실행 가능**: Phase A, B, E는 서로 의존성 없음. 병렬 진행 가능.
**즉시 실행 가능**: Phase A (Expo 55), Phase B (테스트).
**대기**: Phase C (Vercel Pro 전환 또는 SRI 안정화 시), Phase E (검색 품질 피드백 시).
...
|---|---|---|
| Phase 1-9 | - | 즉시 실행 가능 |
| Phase 10-13 | - | 단기 실행 (v1 Phase 9 이후, Phase 13 보류) |
| 범위 외: Expo 55 | Phase A | ✅ 승인, 즉시 실행 가능 |
| 범위 외: Hook/컴포넌트 테스트 | Phase B | ✅ 승인, Tier 1-2부터 점진적 |
| 범위 외: nonce CSP | Phase C | 보류 (Hobby 플랜, SRI 안정화 대기) |
