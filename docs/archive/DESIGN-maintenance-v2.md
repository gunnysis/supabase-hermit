...
> **리팩토링 가이드 반영: 2026-03-12 | Phase G-J 추가 — 성능(FlashList/expo-image/MMKV), Realtime 점검, querySupabase 래퍼, Feature-based 구조 ([리팩토링 가이드](../memo/hermit-comm(은둔마을)%20대규모%20리팩토링%20실전%20가이드.md))**
...
| F7 | 앱 성능 최적화 (FlashList/expo-image/MMKV) | 리팩토링 가이드 | Phase G |
| F8 | Realtime + Supabase 클라이언트 점검 | 리팩토링 가이드 | Phase H |
| F9 | querySupabase 래퍼 + 에러 핸들링 체계화 | 리팩토링 가이드 | Phase I |
| F10 | Feature-based 아키텍처 전환 | 리팩토링 가이드 | Phase J |
...
## 8. Phase G: 앱 성능 최적화 — ANR 해소 (리팩토링 가이드)

> 추가: 2026-03-12 | [리팩토링 실전 가이드](../memo/hermit-comm(은둔마을)%20대규모%20리팩토링%20실전%20가이드.md) 기반
> Sentry 주간 모바일 에러 101건 중 ANR이 가장 치명적. 세 가지 핵심 조치로 해결.

### 8a. FlatList → FlashList 마이그레이션 (v1 L18)

**근거**: `@shopify/flash-list`는 컴포넌트 재활용(recycling)으로 FlatList 대비 CPU 32%↓, 프레임 타임 29%↓.
...
**적용 대상**: 피드 화면(메인), 검색 결과, 감정별 게시글 목록 등 무한 스크롤 사용처 전체.

**주의**: `estimatedItemSize` 누락 시 경고 + 성능 저하.

### 8b. expo-image 도입 (v1 L13 격상)

**근거**: 이미 `package.json`에 설치되었으나 미사용 (RN 기본 Image 사용 중). 디스크/메모리 캐싱, blurhash 플레이스홀더, 부드러운 트랜지션 제공.
...
<Image
  ...
  recyclingKey={post.id}       // FlashList와 함께 쓸 때 필수 (깜빡임 방지)
  transition={200}
  contentFit="cover"
...
**적용 대상**: `PostCard.tsx:88-92` 외 이미지 표시 전체.

### 8c. AsyncStorage → MMKV 전환 (v1 L21)

**근거**: `react-native-mmkv`는 AsyncStorage보다 ~30배 빠르고 동기 API. 설정값, 캐시, preferences 저장에 최적.

**주의**: Expo Go에서 안 돌아감 → `expo prebuild` + dev client 필요 (EAS Build 사용 중이므로 무문제).
...
// 동기 API — AsyncStorage와 달리 await 불필요
...
| 항목 | 트리거 |
|---|---|
| FlashList | ANR 재발 또는 게시글 100건+ 시 |
| expo-image | R5 디자인 개선 시 함께 진행 가능 |
| MMKV | 앱 스토리지 성능 이슈 시, 또는 다음 네이티브 빌드 시 |
...
**위험**: `detectSessionInUrl: false`를 빠뜨리면 Realtime에서 delete 이벤트만 수신되는 심각한 버그 (Supabase GitHub #26980).

**확인 방법**:
```bash
grep -r "detectSessionInUrl" --include="*.ts" --include="*.tsx"
```
...
export const supabase = createClient(url, key, {
  auth: {
    detectSessionInUrl: false,  // ← 네이티브앱 필수!
    // ...
  },
});
```

### 9b. Realtime 구독 cleanup 점검 (v1 L19)

**위험**: `supabase.removeChannel()`을 빠뜨리면 채널이 계속 쌓이면서 메모리 누수 → ANR.

**점검 방법**: 모든 Realtime 관련 `useEffect`에서 cleanup 함수가 `supabase.removeChannel(channel)`을 호출하는지 확인.
...
**AppState 리스너도 cleanup 필수**:
```typescript
const appSub = AppState.addEventListener('change', handler);
return () => {
...
### 9c. expo-sqlite localStorage 폴리필 (최신 패턴)

Expo 55+에서 권장하는 Supabase auth storage:
```typescript
import 'expo-sqlite/localStorage/install';
// → localStorage 폴리필이 자동 제공, auth.storage로 사용
```

현재 `@react-native-async-storage/async-storage`를 쓰고 있다면 교체 검토.

### 9d. 트리거 조건

- **9a/9b는 즉시 확인** — 1분이면 가능, ANR의 유력 원인
- 9c는 Expo 55 업그레이드(v1 R1) 시 함께 진행

---

## 10. Phase I: querySupabase 래퍼 + 에러 핸들링 체계화 (리팩토링 가이드)

> 추가: 2026-03-12 | Sentry unhandled rejection 해소. v1 Phase 3 에러 처리 통일과 보완 관계.

### 10a. querySupabase 래퍼 패턴 (v1 L23)

**근거**: Sentry에서 `APIError: 게시글을 찾을 수 없습니다.`가 onunhandledrejection으로 잡힘 → 비동기 호출 체인에서 에러 누수.

**구현**:
```typescript
// src/lib/api-error.ts (또는 기존 APIError 클래스 확장)
export async function querySupabase<T>(
  queryFn: () => Promise<{ data: T | null; error: any }>
): Promise<T> {
  const { data, error } = await queryFn();
  if (error) {
    throw new APIError(error.message || '알 수 없는 오류', error.code || 'UNKNOWN', error.status);
  }
  if (!data) {
...
v1 Phase 3은 기존 API 파일별 에러 처리 통일 (`extractErrorMessage` + `logger.error`).
Phase I는 그 위에 **Supabase 호출 래퍼**를 추가하여 에러 누수를 원천 차단.

**실행 순서**: v1 R2 (Phase 3) → Phase I (래퍼 도입) — 기존 패턴 정리 후 래퍼 추가.

### 10c. 트리거 조건

- v1 Release 2 완료 후
- 또는 Sentry unhandled rejection이 지속적으로 발생 시

---

## 11. Phase J: Feature-based 아키텍처 전환 (리팩토링 가이드)

> 추가: 2026-03-12 | 가장 큰 구조적 개선. 웹은 이미 `src/features/` 패턴 사용 중.
> **대규모 리팩토링** — v1/v2의 단기 유지보수와는 별도의 마일스톤으로 관리.
...
1. **라우트 파일은 한 줄만**: `export { FeedScreen as default } from '@/features/feed/feed-screen';`
2. **barrel export 금지**: React Native에서 Fast Refresh 문제 유발
3. **웹 앱과 동일 패턴**: `src/features/` 구조로 코드/패턴 공유 극대화
4. **점진적 마이그레이션**: 파일 단위로 이동, 한 feature씩 완성

### 11c. Zustand auth 상태 관리 (선택)

현재 React Context 기반 auth라면 Zustand 전환으로 리렌더링 감소:
```typescript
// src/features/auth/use-auth-store.ts
export const useAuthStore = create<AuthStore>((set) => ({
  session: null, user: null, initialized: false, isAnonymous: false,
  initialize: () => {
...
```
Phase 1 — lib/ 인프라 분리 (1일)
  supabase.ts, api-error.ts, hooks/ 이동
  → import 경로 업데이트

Phase 2 — features/ 1개 파일럿 (1일)
  feed/ feature 생성, FeedScreen + api.ts + components/ 이동
  → app/(tabs)/index.tsx를 re-export로 전환

Phase 3 — 나머지 features/ (2-3일)
  auth/, community/, settings/ 순차 이동

Phase 4 — 정리 (0.5일)
  빈 디렉토리 삭제, 경로 alias 확인, 테스트 통과
```

### 11e. 트리거 조건

- 앱 파일 수 50개+ 도달 시
- 또는 새 기능 추가 시 구조 혼란 발생 시
- 또는 v1 Release 5 완료 후 여유 시
...
Phase F  디자인 시스템 문서화            ← 대기 (팀 확장 시 실행, 보류 적절)
    ↓
Phase G  앱 성능 (FlashList/expo-image/MMKV) ← ANR 재발 시 (Quick Win)
    ↓
Phase H  Realtime + Supabase 클라이언트  ← **즉시 확인 가능** (1분, ANR 유력 원인)
    ↓
...
**독립 실행 가능**: Phase A, B, E, F, G, H는 서로 의존성 없음. 병렬 진행 가능.
**조건부 대기**: Phase C (Vercel Pro 전환 시), Phase E (검색 품질 피드백 시), Phase F (팀 확장 시).
**순차 의존**: Phase I → v1 R2 후, Phase J → v1 R5 후.
**즉시 확인**: Phase H (detectSessionInUrl + Realtime cleanup) — 1분, ANR 유력 원인.

---

## 14. v1 ↔ v2 관계 (리뷰 반영 + 리팩토링 가이드 반영)
...
| v1 L18: FlatList → FlashList | Phase G | ANR 재발 또는 게시글 100건+ 시 |
| v1 L13: expo-image 미사용 | Phase G | R5 디자인 개선 시 함께 |
| v1 L21: AsyncStorage → MMKV | Phase G | 스토리지 성능 이슈 시 |
